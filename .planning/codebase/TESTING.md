...
| Phase 4.1 推荐函数冒烟 | `npx tsx -e ...` | ✅ 覆盖 fallback / L1 / history 三条推荐路径 |
...
| P1 | `utils/recommendation.ts` | mastery 聚合、minimal pair 更新、L1+history 排序、无历史降级 |
...
| P1 | 无 recommendation 单元测试 | mastery 聚合或推荐权重回归只能靠手动冒烟发现 | 引入 Vitest 后添加普通拼写/minimal pair fixture |
