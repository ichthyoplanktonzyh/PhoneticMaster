...
| Structured Input | 按语言结构组合记号的输入辅助 | `StructuredPinyinInput`, `pinyinBuilder` |
...
| 当前文件 | `src/utils/judge.ts`, `src/utils/trainingSession.ts`, `src/utils/minimalPairs.ts`, `src/components/PhonemeDiffView.tsx`, `src/components/SessionResultView.tsx`, `src/components/MinimalPairView.tsx` |
...
| `src/utils/pinyinBuilder.ts` | Language input helper | 生成中文 tone-number 正字法，保持与 parser 语义分离 |
...
- Phase 3.1：将 `TrainingTopic` 从 `selectedPhoneme` 扩展到 minimal pair / tone / final 等结构化主题。
