...
一款面向美式英语学习者的国际音标（IPA）训练工具，提供**拼写训练**和**听力训练**两种模式。通过浏览器原生语音合成听取单词发音，在交互式 IPA 键盘辅助下完成音标转写或进行针对性听力训练，即时获得反馈。

## 功能特性

- **双模式训练**：拼写模式（听音写音标）+ 听力模式（看词听音，按音标分类训练）
- **音标分类听力训练**：按 40 个音素分类，针对薄弱音标进行专项听力训练，Space 键绑定播放
...
- **即时反馈**：拼写模式下提交答案后立即显示对错，错误时展示正确答案和实际单词
...
点击 Header 左侧的「拼写 / 训练」按钮切换模式：

- **拼写模式**：传统的听音写音标训练，听到单词发音后在输入框中拼写 IPA 转写
- **训练模式**：针对特定音标的听力训练，同时展示单词拼写和音标，专注听音辨音
...
1. **切换模式**：点击 Header 的「训练」按钮进入听力训练模式。
2. **选择音标**：通过 Phoneme 下拉菜单选择要训练的目标音标（如 `/eɪ/`、`/θ/`），共 40 个音素可选。选「All」则使用全部单词。
3. **看词听音**：页面展示当前单词的拼写和美式 IPA 音标，点击播放按钮或按键盘 **Space 键** 即可听到发音。
4. **反复聆听**：不限次数重复播放，仔细对比单词拼写、音标符号和实际发音之间的关系。
5. **下一词**：点击「Next Word」或按 Space 键后继续，进入下一个单词。
6. **换一批**：点击底部「New Word Set」重新随机抽取 10 题。

### 音标过滤

Header 右侧的 Phoneme 下拉菜单支持按音标筛选训练词库。选择特定音标后，拼写和训练模式均只使用包含该音标的单词。右侧显示各音标的单词数量统计，方便了解训练材料规模。
...
src/
├── main.tsx                 # 入口
├── App.tsx                  # 主应用组件（模式切换、状态管理）
├── index.css                # 全局样式 + IPA 字体定义
├── types.ts                 # 类型定义
├── components/
│   ├── IPAKeypad.tsx        # IPA 交互键盘组件
│   └── TrainingView.tsx     # 听力训练模式组件
├── data/
│   └── wordBank.ts          # 本地词库（4,648 词）
└── utils/
    ├── voice.ts             # 语音管理（智能选择 + localStorage 持久化）
    ├── ipaParser.ts         # IPA 音标解析器（音素分词）
    └── phonemeGroups.ts     # 音素分组工具（按音标分类单词）
```
